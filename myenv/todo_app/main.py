...
@app.get("/users/{user_id}", response_model=schemas.UserWithTasks)
...
@app.get("/admin/users", response_model=List[schemas.UserWithTasks])
def admin_get_all_users_with_tasks(db: Session = Depends(get_db)):
    return crud.get_users(db)
